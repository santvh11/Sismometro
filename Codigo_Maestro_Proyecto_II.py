...
def factores_acople(params: SectionParams):
    ...
    B_x = np.abs(
        ((params.e_total - bloque_raiz) / (bloque_raiz * (params.e_total**2)))
        * (3 / 2)
    ...
    Integral_linea = (1 / Radio_total**3) - (
        1 / (Radio_total**2 + (Paso * 2 * np.pi + params.N_sub_c_total) ** 2) ** 3 / 2
    )
...
def graficar_resultados(
    t, y_matrix, y_estacionario, titulo, G_sub_A, G_sub_L, params, solver_rk=False
):
    ...
    for i in range(4):
        ...
        ax.text(
            0.02,
            0.02,
            f"Máx: {np.max(np.abs(y_matrix[i])):.2e}",
    ...
    plt.step(
        t_filtro, FEM_Ohm[: len(t_filtro)], where="post", color="purple", linewidth=1
    )
...
def simular_mk2(params: SectionParams, G_sub_L: float, G_sub_A: float):
    ...
    def state_matrix(t, S):
        ...
        dz_dt = v
        dv_dt = (1 / params.m) * (
            (-params.k * z) - (params.c * v * np.abs(v)) + F_ext + (I * G_sub_L)
        )
        dQ_dt = I
    ...
    if solver.success:
        ...
        graficar_resultados(
            ...
            solver.y,
            None,
            "Detector de Gaia MK2 (No Lineal)",
            G_sub_A,
            G_sub_L,
            params,
            True,
...
def obtener_configuracion_usuario():
    while True:
        modelo = input(
            "Modelo a usar - (1) MK1, (2) MK2, (3) Voltaje DAQ en tiempo real: "
        ).strip()
        if modelo in ["1", "2", "3"]:
            break
    ...
    usar_teoricos = (
        input("¿Usar parámetros teóricos? (Enter para Sí, otra tecla para No): ") == ""
    )
    ...
    if tipo_fuerza == 2:
        while True:
            tipo_amplitud = input(
                "Amplitud - (1) Directa, (2) Base a frecuencia: "
            ).strip()
    ...
    if usar_teoricos:
        while True:
            tipo_iman = input(
                "Tipo de Imán - (1) Acero 440, (2) Neodimio N32, (3) Neodimio N35: "
            ).strip()
...
if __name__ == "__main__":
    modelo_seleccionado, usar_teo, fuerza, amplitud, iman = (
