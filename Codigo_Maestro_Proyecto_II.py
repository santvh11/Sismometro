...
#----------------------------------------------------------------------------------------------------
    ...
    if (modelo_mk2== False and simular==True):
        ...
        linea_t, = ax_tiempo.plot([], [], lw=1.5, color='purple', label='Voltaje Filtrado')
        ax_tiempo.set_title("Dominio del Tiempo (Señal Acoplada en AC)")
        ax_tiempo.set_ylabel("Voltaje (V)")
        ax_tiempo.set_xlabel("Tiempo (s)")
        ax_tiempo.grid(True)
        texto_metricas = ax_tiempo.text(0.02, 0.80, '', transform=ax_tiempo.transAxes, 
        ...
        def actualizar(frame):
            ...
            if len(buffer_t) == TAMANO_VENTANA:
                ...
                if estado["contador_frames"] % 10 == 0:
                    try:
                        popt, pcov = curve_fit(modelo_gemelo_digital, t_arr, v_filtrado, p0=[c_sub_lambda], bounds=(0, 10))
                ...
                ax_tiempo.set_xlim(t_arr[0], t_arr[-1]) # Desplazamiento temporal continuo
