...
while True:
    modelo = input("Escriba (1) para mk_1 o (2) para mk_2, o (3) para ver voltaje en tiempo real: ").strip()
    if modelo == "1":
...
class SectionParams:
    ...
    e_sub_cs: float = 0.079*(1e-3)                     #Grosor del Cable del solenoide (diámetro) #Usamos un AGW 40 
...
#--------------------------------------------------------------------------------------------------
        ...
        self.L_sub_s = self.N_sub_c_total*(self.e_total*2*np.pi)        #Longitud del cable solenoide
        self.A_sub_s = (self.N_sub_c*(self.N_sub_c_capas*
        (4*(np.pi**2))*(self.e_total*self.e_sub_cs)))                                  #Área del cable de solenoide
        self.A_effec= (8*self.e_sub_cs*np.pi*self.e_total
        *self.N_sub_c_total)                                              #Área efectiva de contacto con campo magnético en el eje z
        ...
        self.rho = self.rho_0 - self.beta_rho * (self.temp - 273.15)   # [kg/m³], temp en Kelvin
...
def Factores_Acople (params:SectionParams)-> float: 
    ...
    Integral_linea=(1/Radio_total**3)-(1/(Radio_total**2+(Paso*2*np.pi+N_sub_cs_total)**2)**3/2)
...
#-------------------------------------------------------------------------------------------------- 
    if (modelo_mk1==True) :
        ...
        X_t_h= np.array([[z_0, z_dot_0, Q_0, Q_dot_0]]).reshape(4,1)   #Defininimos una condición inicial homogénea para el vector de condiciones iniciales
...
#----------------------------------------------------------------------------------------------------
        ...
        for i in range(4):
            ...
            # Graficamos
            ax.plot(solver.t, solver.y[i] , '--', color=paleta_colores[i], alpha=0.4, label="Solución RK 45")
            ...
            # Anotar valores máximos 
            ax.text(0.95, 0.02, f"Pico: {np.max(np.abs(solver.y[i])):.2e}", transform=ax.transAxes,
                ha='right', fontsize=9, bbox=dict(facecolor='white', alpha=0.7))
    ...
    if (modelo_mk2== False and simular==True):
        ...
        # ==========================================
        # 2.3. DEFINICIÓN DE MATEMÁTICA Y FILTROS
        # ==========================================
        ...
        # ==========================================
        # 3.3 PREPARACIÓN DE LA INTERFAZ GRÁFICA
        # ==========================================
        try:
        ...
        # ==========================================
        # 4.3 FUNCIÓN MATEMÁTICA PARA INGENIERÍA INVERSA
        # ==========================================
        # Esta función usa k, m, F_0, omega, G_sub_A y R_porcentaje que ya están desempaquetados 
        # en las primeras líneas de la función Solver()
        ...
        def modelo_gemelo_digital(t_datos, c_ajuste):
            ...
            FEM_FL_teorica = G_sub_A * R_porcentaje * velocidad_teorica*factor_amplificacion
            return FEM_FL_teorica
        
        # ==========================================
        # 5.3 MOTOR PRINCIPAL DE PROCESAMIENTO
        # ==========================================
        def actualizar(frame):
            ...
            if len(buffer_t) == TAMANO_VENTANA:
                ...
                try:
                    # popt guardará el valor de 'c' que hace que el voltaje real coincida con la FEM_FL_teorica
                    # Ponemos límites: 'c' no puede ser negativo, y le damos un máximo razonable (ej. 10)
                    popt, pcov = curve_fit(modelo_gemelo_digital , t_arr, v_filtrado, p0=[0.43], bounds=(0, 10))
                    c_estimado = popt[0]
                except:
