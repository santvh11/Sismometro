...
while True:
    ...
    if modelo == "1":
        modelo_mk_1, modelo_mk_2, simular, datos_descarga = True, False, False, False
        break  # Sale del bucle y sigue el programa
    elif modelo == "2":
        modelo_mk_1, modelo_mk_2, simular, datos_descarga = False, True, False, False
        break
    elif modelo == "3":
        modelo_mk_1, modelo_mk_2, simular, datos_descarga = True, False, True, False
        break
    elif modelo == "4":
        modelo_mk_1, modelo_mk_2, simular, datos_descarga = True, False, False, True
        break
    else:
...
def Solver(modelo_mk1: bool, modelo_mk2: bool, params: SectionParams) -> float:
    ...
    if modelo_mk1 == True:
        ...
        if simular == False and datos_descarga == False:
    ...
    if modelo_mk2 == False and simular == True:
        ...
        plt.show()
        esp32.close()
    if modelo_mk_1 == True and datos_descarga == True:
        ...
        def registrar_daq_headless(params, G_sub_A: float):
            ...
            # Buffers estándar en lugar de deque para limpiar fácilmente
            buffer_t = []
            buffer_v = []
                ...
                while True:
                    while esp32.in_waiting > 0:
                        try:
                            ...
                            continue  # Ignorar líneas corruptas en silencio

                    # Procesar bloque solo cuando esté lleno
                    if len(buffer_t) >= TAMANO_VENTANA:
                        t_arr = np.array(buffer_t[:TAMANO_VENTANA])
                        v_arr = np.array(buffer_v[:TAMANO_VENTANA])

                        # Limpiar buffers para recolectar el siguiente bloque inmediatamente
                        ...
                        # Feedback visual rápido en terminal
                        print(
